...
macro AutoExpand()
{
    ...
    nTabCount = GetReg(TABCOUNT)
    szTab = CreateBlankString(nTabCount)
    
    szLine1 = strmid(szLine,0,ich)
    szLine = strmid(szLine, 0, ich) # "@szTab@"
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "for")
    {
        ...
        szInit = ask("Please input init variable")
        szCount = ask("Please input loop count")
        newsel = sel
        newsel.ichLim = GetBufLineLength (hbuf, ln)
        SetWndSel(hwnd, newsel)
        SetBufSelText(hbuf, " (@szVar@ = @szInit@; @szVar@ < @szCount@; @szVar@++)")
    }
    else if (szCmd == "fo")
    {
    ...
    {
        szComCount = ask("Please input comment count:")
        if(IsNumber(szComCount))
        {
            szComCount = szComCount - 1
            DelBufLine(hwnd, ln)
            sel.lnLast = ln + szComCount
            sel.ichLim = strlen(GetBufLine(hbuf,ln + szComCount))
            SetWndSel(hwnd, sel)
            MultiLineComment()
        }
    }
...
macro GetSecondWord(szLine)
{
    ...
    while(nIdx < iLen)
    {
          ...
          || (szLine[nIdx] == ",") || (szLine[nIdx] == ":") )
        {
            ...
            {
                return strmid(szLine,nIdx1 + 1,nIdx)
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 6,  " * File Name     : @sz@")
    ...
    InsBufLine(hbuf, ln + 8,  " * Date          : @sz@-@szMonth@-@szDay@")
    ...
    InsBufLine(hbuf, ln + 13, " * Record        :")
    InsBufLine(hbuf, ln + 14, " * 1.Date        : @sz@-@szMonth@-@szDay@")
    InsBufLine(hbuf, ln + 15, " *   Author      : @szName@")
    InsBufLine(hbuf, ln + 16, " *   Modification: Created file")
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 5,  " *")
    sz = GetFileName(GetBufName (hbuf))
    InsBufLine(hbuf, ln + 6,  " * 文件名称      : @sz@")
    InsBufLine(hbuf, ln + 7,  " * 作者名称      : @szName@")
    SysTime = GetSysTime(1)
    sz=SysTime.Year
    sz1=SysTime.month
    sz3=SysTime.day
    ...
    InsBufLine(hbuf, ln + 8,  " * 创建日期      : @sz@-@szMonth@-@szDay@")
    ...
    InsBufLine(hbuf, ln + 9, " * 版本信息      : 1.0")
    InsBufLine(hbuf, ln + 10," * 描述信息      : @szContent@")
    ...
    InsBufLine(hbuf, ln + 13, " * 修改记录      :")
    InsBufLine(hbuf, ln + 14, " * 1.日 期       : @sz@-@szMonth@-@szDay@")
    InsBufLine(hbuf, ln + 15, " *   作 者       : @szName@")
    InsBufLine(hbuf, ln + 16, " *   修 订       : 创建文件")
    InsBufLine(hbuf, ln + 17, " *")
    InsBufLine(hbuf, ln + 18, "********************************************************************************/")
    InsBufLine(hbuf, ln + 19, "")
    ...
    InsBufLine(hbuf, ln + 21, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 22, "/*                                                                          */")
    InsBufLine(hbuf, ln + 23, "/*                              包含文件                                    */")
    InsBufLine(hbuf, ln + 24, "/*                                                                          */")
    InsBufLine(hbuf, ln + 25, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 26, "")
    InsBufLine(hbuf, ln + 27, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 28, "/*                                                                          */")
    InsBufLine(hbuf, ln + 29, "/*                               宏定义                                     */")
    InsBufLine(hbuf, ln + 30, "/*                                                                          */")
    InsBufLine(hbuf, ln + 31, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 32, "")
    InsBufLine(hbuf, ln + 33, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 34, "/*                                                                          */")
    InsBufLine(hbuf, ln + 35, "/*                              类型定义                                    */")
    InsBufLine(hbuf, ln + 36, "/*                                                                          */")
    InsBufLine(hbuf, ln + 37, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 38, "")
    InsBufLine(hbuf, ln + 39, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 40, "/*                                                                          */")
    InsBufLine(hbuf, ln + 41, "/*                              内部函数                                    */")
    InsBufLine(hbuf, ln + 42, "/*                                                                          */")
    InsBufLine(hbuf, ln + 43, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 44, "")
    InsBufLine(hbuf, ln + 45, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 46, "/*                                                                          */")
    InsBufLine(hbuf, ln + 47, "/*                              全局变量                                    */")
    InsBufLine(hbuf, ln + 48, "/*                                                                          */")
    InsBufLine(hbuf, ln + 49, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 50, "")
    InsBufLine(hbuf, ln + 51, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 52, "/*                                                                          */")
    InsBufLine(hbuf, ln + 53, "/*                              文件变量                                    */")
    InsBufLine(hbuf, ln + 54, "/*                                                                          */")
    InsBufLine(hbuf, ln + 55, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 56, "")
    InsBufLine(hbuf, ln + 57, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 58, "/*                                                                          */")
    InsBufLine(hbuf, ln + 59, "/*                              外部函数                                    */")
    InsBufLine(hbuf, ln + 60, "/*                                                                          */")
    InsBufLine(hbuf, ln + 61, "/****************************************************************************/")
    InsBufLine(hbuf, ln + 62, "")
    InsBufLine(hbuf, ln + 63, "")
    ...
    //如果没有功能描述内容则提示输入
    szContent = Ask("请输入描述信息")
    SetBufIns(hbuf,nlnDesc + 14,0)
    DelBufLine(hbuf,nlnDesc +10)
    
    //注释输出处理,自动换行
    CommentContent(hbuf,nlnDesc + 10," * 描述信息      : ",szContent,0)
}
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if(newFunc != 1)
    {
        ...
        if(strlen(symbol) > 0)
        {
            ...
            if(szRet == "static")
            {
                szTemp = TrimString(szLine)
                szRet = GetSecondWord(szTemp)
            }
            // JiaweiXu 20171127
            if(symbol.Type == "Method")
            {
    ...
    {
        InsBufLine(hbuf, ln+10, "VOS_UINT32  @szFunc@( # )")
        InsBufLine(hbuf, ln+11, "{");
        InsBufLine(hbuf, ln+12, "@szTab@#");
...
macro FuncHeadCommentEN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        InsBufLine(hbuf, ln+9, "VOS_UINT32  @szFunc@(#)")
        InsBufLine(hbuf, ln+10, "{");
        InsBufLine(hbuf, ln+11, "@szTab@#");
        InsBufLine(hbuf, ln+12, "}");
        SearchForward()
    }        
    hwnd = GetCurrentWnd()
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        ...
        if(strlen(szRet) > 0)
        {
            ...
            PutBufLine(hbuf, ln+9, "@szRet@ @szFunc@(#)")
            SetbufIns(hbuf,ln+9,strlen(szRet)+strlen(szFunc) + 3
        }
        szFuncDef = ""
        isFirstParam = 1
...
//    }
    if(language == 0)
    {
        iHistoryCount = Ask("请输入历史记录编号")
    
        InsertHistoryContentCN(hbuf,ln,iHistoryCount)
    }
    else
    {
        iHistoryCount = Ask("Please input history number")
...
macro InsertHistoryContentCN(hbuf,ln,iHostoryCount)
{
    ...
    szMyName = getreg(MYNAME)
    InsBufLine(hbuf, ln, " *")
    InsBufLine(hbuf, ln + 1, " * @iHostoryCount@.日 期       : @sz1@-@szMonth@-@szDay@")

    InsBufLine(hbuf, ln + 2, " *   作 者       : @szMyName@")
    szContent = Ask("请输入修改信息")
    CommentContent(hbuf,ln + 3," *   修 订       : ",szContent,0)
}
...
macro ReplaceBufTab()
{
    ...
    if(language == 0)
    {
        nBlank = Ask("请输入Tab替换空格数:")
    }
    else
    {
        nBlank = Ask("Please input the blank count of Tab:")
    }
    if(nBlank == 0)
    {
        nBlank = 4
...
macro ConfigureSystem()
{
    ...
    if(szLanguage == 1)
    {
        szTabCount = ASK("Please input the count of Tab:");
    }
    else
    {
        szTabCount = ASK("请输入制表符空格数:");
    }
    if(IsNumber(szTabCount))
    {
       SetReg ("TABCOUNT", szTabCount)
    }
    else
    {
       SetReg ("TABCOUNT", 4)
...
macro ExpandBraceLarge()
{
    ...
    if(sel.lnFirst == sel.lnLast && sel.ichFirst == sel.ichLim)
    {
        ...
        InsBufLine(hbuf, ln + 1, "@szLeft@@szTab@")
        InsBufLine(hbuf, ln + 2, "@szLeft@}")
        nlineCount = nlineCount + 2
        SetBufIns (hbuf, ln + 1, strlen(szLeft)+nTabCount)
    }
    else
    {
        ...
        while( nIdx < sel.lnLast)
        {
            ...
            szCurLine = cat("@szTab@",szCurLine)
            if(nIdx == sel.lnLast - 1)
            {
                //对于最后一行应该是选中区内的内容后移四位
                szCurLine = strmid(szCurLine,0,szLineselichLim + nTabCount)
        ...
        {
            //插入第一行选择区的内容
            InsBufLine(hbuf, ln+1, "@szLeft@@szTab@@szMid@")
            nlineCount = nlineCount + 1
            ln = ln + 1
        }        
    }
...
macro InsertElse()
{
    ...
    if(sel.lnFirst == sel.lnLast && sel.ichFirst == sel.ichLim)
    {
        PutBufLine(hbuf,ln+2, "@szLeft@@szTab@")
        SetBufIns (hbuf, ln+2, strlen(szLeft)+nTabCount)
        return
    }
...
macro InsertCase()
{
    ...
    InsBufLine(hbuf, ln + 1, "@szLeft@" # "{")
    InsBufLine(hbuf, ln + 2, "@szLeft@" # "@szTab@" # "#")
    InsBufLine(hbuf, ln + 3, "@szLeft@" # "@szTab@" # "break;")
...
//            nLeft = GetLeftBlank(szLine)
            ...
            {
                ln = ln + 6
                InsBufLine(hbuf, ln - 4, "@szLeft@@szTab@" # "case @szLine@ :")
                InsBufLine(hbuf, ln - 3, "@szLeft@@szTab@" # "{")
                InsBufLine(hbuf, ln - 2, "@szLeft@@szTab@" # "@szTab@" # "#")
                InsBufLine(hbuf, ln - 1, "@szLeft@@szTab@" # "@szTab@" # "break;")
                InsBufLine(hbuf, ln    , "@szLeft@@szTab@" # "}")
                InsBufLine(hbuf, ln + 1, "@szLeft@@szTab@" # "")
            }
            i = i + 1
         }
     ...
     else
     {
         ...
         {
            ln = ln + 6
            InsBufLine(hbuf, ln - 4, "@szLeft@@szTab@" # "case # :")
            InsBufLine(hbuf, ln - 3, "@szLeft@@szTab@" # "{")
            InsBufLine(hbuf, ln - 2, "@szLeft@@szTab@" # "@szTab@" # "#")
            InsBufLine(hbuf, ln - 1, "@szLeft@@szTab@" # "@szTab@" # "break;")
            InsBufLine(hbuf, ln    , "@szLeft@@szTab@" # "}")
    ...
    InsBufLine(hbuf, ln + 2, "@szLeft@@szTab@" # "")
    InsBufLine(hbuf, ln + 3, "@szLeft@@szTab@" # "default:")
    InsBufLine(hbuf, ln + 4, "@szLeft@@szTab@" # "{")
    InsBufLine(hbuf, ln + 5, "@szLeft@@szTab@" # "@szTab@" # "#")
    InsBufLine(hbuf, ln + 6, "@szLeft@@szTab@" # "@szTab@" # "break;")
    InsBufLine(hbuf, ln + 7, "@szLeft@@szTab@" # "}")
    InsBufLine(hbuf, ln + 8, "@szLeft@" # "}")
    SetWndSel(hwnd, sel)
    SearchForward()
}
...
macro InsertDo()
{
    ...
    if(sel.lnFirst == sel.lnLast && sel.ichFirst == sel.ichLim)
    {
        PutBufLine(hbuf,ln+1, "@szLeft@@szTab@#")
    }
...
macro InsertFor()
{
    ...
    PutBufLine(hbuf,ln, "@szLeft@for(@szVar@ = @szInit@; @szVar@ < @szCount@; @szVar@++)")
    SearchForward()
}
...
macro MultiLineComment()
{
    ...
    while(Ln <= Lnlast) 
    {
        ...
        if(len >= lnLeft + 1)
        {
            if(len >= lnLeft + 2)
            {
                if(StrMid(buf, lnLeft, lnLeft + 1) == "/") 
                {       //需要取消注释,防止只有单字符的行
                    if(StrMid(buf, lnLeft + 1, lnLeft + 2) == "/")
                    {
                        PutBufLine(hbuf, Ln, StrMid(buf, 0, lnLeft) # StrMid(buf, lnLeft + 2, len))
                    }
                }
            }
            ...
            if(StrMid(buf,lnLeft ,lnLeft + 1) != "/")
            {          //需要添加注释
                PutBufLine(hbuf, Ln, StrMid(buf, 0, lnLeft) # "//" # StrMid(buf, lnLeft, len))
            }
        }
        Ln = Ln + 1
    }
    selection.ichFirst = 0
    selection.ichLim = selection.ichLim + 2
    SetWndSel(hwnd, selection)
}
/*****************************************************************************
 函 数 名  : SuperBackspace
 功能描述  : 删除汉字
 输出参数  : 无
 返 回 值  : 
 调用函数  : 
